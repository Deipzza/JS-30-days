...
function strip(bandName) {
    return bandName.replace(/^(a\s|an\s|the\s)/i, '').trim();
}

bandsSorted = bands.sort((a, b) => (strip(a) > strip(b)) ? 1 : -1);

document.querySelector("#bands").innerHTML = bandsSorted.map(band => `<li>${band}</li>`).join('');